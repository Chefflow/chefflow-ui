...
Guidance for Claude Code working in this repo. **The harness workflow below is mandatory.**

## Harness workflow (spec-driven)

This repo runs a spec-driven development (SDD) loop. Do not free-style features.

1. **Run the Guardian first.** Before any task: `./init.sh`. It must exit green
   (toolchain, deps, `tsc --noEmit`, Biome). Never build on a red baseline.
2. **Backlog:** `features.json` tracks every feature (`pending → spec_ready →
   in_progress → done`).
3. **Spec is the source of truth:** `specs/requirements.md` (EARS notation),
   `specs/design.md` (files + patterns to touch), `specs/tasks.md` (atomic checklist).
   No code before the spec is filled. One active spec at a time; archive shipped specs
   under `docs/tasks/`.
4. **Specialized agents** (`.claude/agents/`): `leader` orchestrates · `spec-author`
   writes specs · `implementer` codes only what the spec says · `reviewer` validates
   against `init.sh` + conventions and rejects unverified work.
5. **Verification is required.** Every requirement maps to a check; "no verification,
   no merge."
6. **Context hygiene.** Past ~20% context, summarize into `progress/history.md` and
   `/clear`.

## Project

ChefFlow — recipe management & meal planning. Next.js 16 (App Router), React 19,
TypeScript strict, Tailwind v4 + shadcn/ui, next-intl, TanStack React Query, Zustand.
Talks to a separate `chefflow-api` backend (JWT cookie auth).
...
./init.sh           # harness Guardian — run before any task
pnpm dlx shadcn@latest add [name]   # add a shadcn component, then `pnpm run format`
```

## Non-negotiable rules

- **pnpm** only. **Biome** for lint/format (Husky pre-commit enforces it).
- TypeScript: prefer `interface` over `type`; no `any` (use `unknown`); `??` not `||`
  for null/undefined fallback; `import type`; explicit return types on public functions.
- **No comments** unless strictly necessary. Code and comments in English.
- Naming: `.tsx` PascalCase in its own PascalCase folder; `.ts` camelCase;
  `src/components/ui/` stays kebab-case (shadcn); Next.js reserved files keep names.
  Named exports preferred.
- Components: Server by default (`function`); client islands (`const` + `"use client"`)
  only when needed, kept minimal.
- Data only through React Query hooks (`src/hooks/`) + API clients (`src/lib/api/`);
  throw on `response.error`; toast via `sonner`. Never `fetch` from components.
- Styling: shadcn/ui + Tailwind utilities; theme tokens in `src/app/globals.css`
  (`@theme`); use `cn()` from `@/lib/utils`. Path alias `@/*` → `./src/*`.
- i18n: every user-facing string needs keys in all `messages/{en,es,fr,de,it}.json`.

## Reference docs

- `docs/conventions.md` — full code style + the repo's real patterns (React Query, API
  clients, Zustand, forms, i18n). The implementer's bible.
- `docs/architecture.md` — file structure, i18n setup, integration points.
- `docs/design-system.md` — colors (OKLCH), typography, spacing/effects.
